...
# when creating vectors of numerical values (integers) you should NOT use the quotation marks around each element 
a <- c(50, 60, 50)
b <- c(1, 2, 2)
c <- c(4, 5, 5)
...
class(a) # class is the larger umbrella 
typeof(a) # specific type within the class numeric 
...
# code for installing a package only needs to be done the first time a package is installed 
# install.packages("DescTools")
# this package will allow you to calculate the Mode of a vector 

# load the library 
# loading a library has to be done every time a package will be used in the code 
library(DescTools)

# mode will not work if there is no value that occurs more than once 
Mode(a)
Mode(b)
Mode(c)
...
# can create a data frame with all 3 vectors 
