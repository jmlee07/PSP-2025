...
```{r}
ncha_demo <- read_xlsx("C:/Users/johnm/OneDrive/Desktop/PINC 2025/PSP 2025/Onramp/Data/NCHA2021_RWS_Subset.xlsx", col_names = TRUE)
```
...
OverallHealth <- ncha_demo$'Overall Health'

Weight <- ncha_demo$'Self described weight'

ModExercise <- ncha_demo$'Minutes Moderate PA - minutes'
```
7.  Alright, now let's create a NEW data frame with the data from just those 3 variables.

```{r}
ncha_new <- data.frame(OverallHealth, Weight, ModExercise)
```

8.  Lastly, let's export that new data frame into a CSV file!

```{r}
write.csv(ncha_new, "C:/Users/johnm/OneDrive/Desktop/PINC 2025/PSP 2025/Onramp/Data/ncha_new.csv")
```
...
```{r}
ncha_activity <- read_csv("C:/Users/johnm/OneDrive/Desktop/PINC 2025/PSP 2025/Onramp/Data/NCHA2021_Session3_5_Subset.csv", col_names = TRUE)
...
1.  How many variables are in the data set? *Hint: you can use either View() or str() to answer this question!* 13 variables

2.  What two types/classes of data are present in the data set? *Hint: use the function str() to answer the questions.* chr and num
...
```{r}
# Input code here
table(ncha_activity$OVRL_HEALTH)
```

Questions:

1.  How many different values does the variable OVRL_HEALTH have? 5 different values
...
```{r}
# Insert code below
ncha_new_activity_ <- data.frame(OVRL_HEALTH, SELFDESC_WEIGHT, )
```
