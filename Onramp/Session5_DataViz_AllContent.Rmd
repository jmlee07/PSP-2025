...
```{r}
ncha <- read.csv(file = "C:/Users/johnm/OneDrive/Desktop/PINC 2025/PSP 2025/Onramp/Data/NCHA2021_Session3_5_Subset.csv", header = TRUE, na.strings=c("","NA"))
```
...
# load in the data
ncha <- read.csv(file = "Data/NCHA2021_Session3_5_Subset.csv", header = TRUE, na.strings=c("","NA"))
...
```{r}
boxplot(MIN_MOD_PA ~ OVRL_HEALTH, data = ncha)
```
...
```{r}
ggplot(ncha, aes(x = AGE, y = MIN_VIG_PA)) +
  geom_point(position = "jitter") +
  xlab("Age") +
  ylab("Minutes of Vigorous Physical Activity") +
  ggtitle("Scatterplot of Age vs. Vigorous PA")
```
...
```{r}
library(ggplot2)
ggplot(ncha) + geom_histogram(aes(x = AGE, fill = OVRL_HEALTH), bins = 15, col = "white") + 
scale_x_continuous(n.breaks = 15) + scale_y_log10() +
scale_fill_brewer(palette = "Set2") + xlab("Age") + ylab("Number of Students") +
ggtitle("Number of Students by Age and Overall Healt") +
theme_dark()
```
